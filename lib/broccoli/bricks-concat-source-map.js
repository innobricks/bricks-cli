...
BroccoliSourceMap.prototype.write = function (readTree, destDir) {
    ...
    return readTree(this.inputTree).then(function (srcDir) {
        ...
        fs.writeFileSync(path.join(destDir, self.outputFile), code_map.code);
        fs.writeFileSync(path.join(destDir, self.outputFile + ".map"), JSON.stringify(code_map.map));
        ...
        function addFile(filePath) {
            ...
            if (cacheObject == null) { // cache miss
                ...
                var childNodeChunks = fileContents.split("\n");
                for (var i = 0; i < childNodeChunks.length; i++) {
                    childNodeChunks[i] += "\n";
                }
                childNodeChunks.forEach(function (line, ind) {
